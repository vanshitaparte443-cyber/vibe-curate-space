...
function FavoritesPage() {
  ...
  const favs = all.filter((b) => vault.favorites.includes(b.id));
  const filtered = search.trim()
    ? searchBoards(search).filter((b) => vault.favorites.includes(b.id))
    : favs;

  return (
    <AppShell onCreate={() => setModalOpen(true)} searchValue={search} onSearchChange={setSearch}>
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">Loved by you</p>
        <h1 className="mt-1 font-display text-3xl text-foreground sm:text-4xl">Favorites</h1>
        <p className="mt-1 text-sm text-muted-foreground">{favs.length} saved {favs.length === 1 ? "board" : "boards"}</p>
        ...
        <div className="mt-10">
          {filtered.length === 0 ? (
            <div className="glass mx-auto grid max-w-xl place-items-center rounded-3xl px-8 py-16 text-center shadow-soft">
              ...
              <p className="mt-4 font-display text-2xl text-foreground">{search ? "Nothing matches." : "No favorites yet."}</p>
              <p className="mt-2 text-sm text-muted-foreground">Tap the heart on any board to save it here.</p>
